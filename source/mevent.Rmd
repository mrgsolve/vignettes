...
was given and schedule the event `1.2` hours into the future.  We keep 
...
There is a function in the `self` object called `mtime()` that takes two 
arguments: first the `TIME` of the event and second the `EVID`.  In this 
example, we create a record with `EVID` of 33 at 1.2 hours after the dose. We 
