...
we'll use the `mread()` function to read, parse, compile and load a model 
out of the internal model library.  Try calling the modlib function (`modlib()`)
and see that the path to the installed package is returned, pointing to a directory
of pre-coded models.  These models are good to try when learning mrgsolve.  
